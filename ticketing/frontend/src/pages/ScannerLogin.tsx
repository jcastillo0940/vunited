...
export function ScannerLogin() {
    ...
    async function handleSubmit(event: FormEvent) {
        ...
        setError(null);
        try {
            const deviceName = `escaner-${navigator.userAgent.slice(0, 30)}`;
            const res = await login(email, password, deviceName);
            storeToken(res.token);
            navigate('/escaner');
        } catch (err) {
            setError(err instanceof ApiError ? 'Credenciales inválidas.' : 'No se pudo conectar. Verifica tu conexión.');
        } finally {
            setSubmitting(false);
        }
    }
        ...
        <div className="flex min-h-screen items-center justify-center bg-primary px-4">
            <Container className="max-w-sm">
                <div className="mb-8 flex flex-col items-center gap-3 text-white">
                    ...
                    <p className="font-display text-lg font-bold uppercase tracking-tight">Escáner de puerta</p>
                </div>
                <Card>
